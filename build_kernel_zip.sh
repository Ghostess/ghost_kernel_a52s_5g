...
make \
    ...
    ARCH=arm64 \
    CROSS_COMPILE=aarch64-linux-gnu- \
    STRIP="${CLANG_DIR}/bin/llvm-strip --strip-debug" \
    INSTALL_MOD_PATH="${MODULES_STAGING}" \
    INSTALL_MOD_STRIP=1 \
    modules_install \
    || die "modules_install failed"

# strip the .ko files (again.. just in case)
find . -path "${MODULES_STAGING}" -name "*.ko" -exec "${CLANG_DIR}/bin/llvm-strip" --strip-unneeded {} \;
